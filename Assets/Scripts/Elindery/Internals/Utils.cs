...
namespace Elindery.Internals
{
    public static class Utils
    {
        ...
        public static void CreateBulletTracer(Vector3 fromPosition, Vector3 toPosition)
        {
            ...
            Material tmpTracerMaterial = new Material(FXProvider.Instance.BulletTracerMaterial);
        ...
        public static async void ButtonClickTimeout(Button button, float timeoutSeconds)
        {
            button.enabled = false;
            await Task.Delay(Mathf.CeilToInt(timeoutSeconds * 1000));
            button.enabled = true;
        }
    }
}
